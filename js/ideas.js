 ...
 function updateLifeButton(){q('#iso-life').textContent=lifeRunning?'暫停街景':'播放街景';q('#iso-life').setAttribute('aria-pressed',String(lifeRunning));}
 function animateLife(now){lifeFrame=0;if(!lifeRunning||!lifeVisible||document.hidden){lastFrame=0;return;}if(!lastFrame)lastFrame=now;if(now-lastFrame>=50){lifeTime+=Math.min(.1,(now-lastFrame)/1000);lastFrame=now;draw();}lifeFrame=requestAnimationFrame(animateLife);}
 function scheduleLife(){if(lifeFrame)cancelAnimationFrame(lifeFrame);lifeFrame=0;lastFrame=0;if(lifeRunning&&lifeVisible&&!document.hidden)lifeFrame=requestAnimationFrame(animateLife);}
 q('#iso-life').onclick=()=>{lifeRunning=!lifeRunning;updateLifeButton();scheduleLife();};motionPreference.addEventListener('change',e=>{lifeRunning=!e.matches;updateLifeButton();scheduleLife();});document.addEventListener('visibilitychange',scheduleLife);
...
function drawCityLandmark(i,x,y){const previousX=ox,previousY=oy;const p=P(x,y,0);ox=p[0];oy=p[1];if(selected===i)poly([[-4,-4,-.1],[118,-4,-.1],[118,98,-.1],[-4,98,-.1]],'#e4dba3');if(!IdeaLandmarks.draw(cards[i],{box,poly,line,circle,face,gear,robot,car,tree,ctx,wall,mint,blue,rose,roof,dark,gold})){if(cards[i].model>=0)landmark(cards[i].model);else genericLandmark(cards[i]);}ox=previousX;oy=previousY;const hit=P(x+53,y+43,45);hits.push({i,x:hit[0],y:hit[1],r:70*scale});}
 ...
 function draw(){const w=canvas.clientWidth,h=canvas.clientHeight,dpr=Math.min(2,window.devicePixelRatio||1);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);}ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);hits=[];const cols=Math.min(count,Math.max(4,Math.ceil(Math.sqrt(ideas.length)))),rows=Math.ceil(count/cols),extentX=cols*155+10,extentY=rows*155+10;scale=zoom*Math.min((w-32)/((extentX+extentY)*.866),(h-95)/((extentX+extentY)*.5+80));ox=w/2-(extentX-extentY)*.433*scale;oy=(h-((extentX+extentY)*.5+55)*scale)/2+70*scale;
  ...
  const objects=[];
  for(let i=0;i<count;i++){const x=i%cols*155+10,y=Math.floor(i/cols)*155+10;objects.push({depth:x+y+180,paint:()=>drawCityLandmark(i,x,y)});if(i%3===0)objects.push({depth:x+y+247,paint:()=>{bench(x+65,y+126);lamp(x+127,y+117);}});}
  ...
  for(const actor of CityLife.actors(cols,rows,count,lifeTime))objects.push({depth:actor.depth,paint:()=>CityLife.paint(actor,{box,line,circle,poly,wall,mint,blue,rose,gold,dark,roof})});
