...
function genericLandmark(card){
 const seed=Array.from(card.source).reduce((a,c)=>(a*31+c.charCodeAt(0))>>>0,0),height=28+seed%18;
...
function drawCityLandmark(i,x,y){const previousX=ox,previousY=oy;const p=P(x,y,0);ox=p[0];oy=p[1];if(selected===i)poly([[-4,-4,-.1],[118,-4,-.1],[118,98,-.1],[-4,98,-.1]],'#e4dba3');if(cards[i].model>=0)landmark(cards[i].model);else genericLandmark(cards[i]);ox=previousX;oy=previousY;const hit=P(x+53,y+43,45);hits.push({i,x:hit[0],y:hit[1],r:70*scale});}
 ...
 function draw(){const w=canvas.clientWidth,h=canvas.clientHeight,dpr=window.devicePixelRatio||1;canvas.width=w*dpr;canvas.height=h*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);hits=[];const rows=Math.ceil(count/4),cols=Math.min(4,count),extentX=cols*155+10,extentY=rows*155+10;scale=zoom*Math.min((w-32)/((extentX+extentY)*.866),(h-95)/((extentX+extentY)*.5+80));ox=w/2-(extentX-extentY)*.433*scale;oy=(h-((extentX+extentY)*.5+55)*scale)/2+70*scale;
  if(focus){const cx=(selected%4)*155+63,cy=Math.floor(selected/4)*155+53;ox=(cardOpen&&w>640?w*.32:w*.5)-(cx-cy)*.8660254*scale;oy=h*(cardOpen&&w<=640?.32:.55)-((cx+cy)*.5-27)*scale;}ox+=panX;oy+=panY;q('#iso-zoom').textContent=Math.round(zoom*100)+'%';
  ...
  const objects=[];for(let i=0;i<count;i++){let col=i%4,row=Math.floor(i/4);objects.push({i,x:col*155+10,y:row*155+10});}objects.sort((a,b)=>a.x+a.y-b.x-b.y).forEach(o=>drawCityLandmark(o.i,o.x,o.y));
 ...
 function selectIdea(i){selected=i;cardOpen=true;focus=true;zoom=canvas.clientWidth<=640?3.5:2.8;panX=panY=0;q('#idea-extra').hidden=true;q('#idea-expand').setAttribute('aria-expanded','false');q('#idea-expand').textContent='更多內容';sync();q('.idea-card').scrollTop=0;save();}
 ...
 function zoomAt(next,x,y){const old=zoom,oldX=ox,oldY=oy;zoom=Math.max(.65,Math.min(6,next));const ratio=zoom/old;draw();panX+=x-(x-oldX)*ratio-ox;panY+=y-(y-oldY)*ratio-oy;draw();}
